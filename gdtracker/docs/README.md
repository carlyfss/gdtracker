...
| `src/components/` | `ProtectedRoute`, `GameDashboardLayout` (header nav + outlet), shared UI (`TaskDescriptionMarkdown`, `FeatureTasksModal`, `TagTasksModal`) |
| `src/pages/`      | `LoginPage`, `GamesHubPage`, `GameCreatePage`, plus dashboard pages (`DashboardPage`, `HeatmapPage`, `TasksPage`, `ConfigurationPage`)     |
| `src/api/`        | Axios `client.ts` (credentials + CSRF header), `auth.ts`, `games.ts`, and game-scoped domain modules                                       |
| `src/util/`       | Pure TypeScript helpers (e.g. tree/feature helpers)—no React imports unless necessary for that helper                                      |
...
| Topic   | Choice                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Layout  | Full-width shell with **16px** horizontal inset on the top header and main content (see [`src/App.css`](../src/App.css)); game routes also include a **left icon sidebar** (Dashboard / Heatmap / Tasks / Configuration) that fills the available height below the header. Sidebar labels appear as overlay tooltips on hover/focus (no layout shift). `.appShell` is a column flex layout so the body fills the viewport below the header. Game dashboard sections use `.gamePageSection` / `.gamePageStack` instead of floating `.card` panels. Games hub column scales with `.gamesHub`. |
| Build   | Vite + TypeScript + React                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Routing | `react-router-dom` — `/login`, `/games`, `/games/new`, `/g/:gameId/{dashboard,heatmap,tasks,configuration}`; legacy `/g/:gameId/exceptions` redirects to `dashboard`; legacy `features` redirects to `configuration`                                                                                                                                                                                                                                                                                                                                                                        |
| HTTP    | `axios` via [`src/api/client.ts`](../src/api/client.ts): `withCredentials: true` for session cookies; `X-XSRF-TOKEN` from `XSRF-TOKEN` cookie for mutating requests (issue a GET such as `listGames()` first so the cookie exists). Dev uses relative `baseURL` (`''`) so `/api` hits the Vite proxy. Production builds use `import.meta.env.VITE_API_BASE_URL` (no hardcoded fallback).                                                                                                                                                                                                    |
| Proxy   | [`vite.config.ts`](../vite.config.ts) proxies `/api` → `VITE_DEV_PROXY_TARGET` (default `http://localhost:8080`) during `npm run dev`.                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
