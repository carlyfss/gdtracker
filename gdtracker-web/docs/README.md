...
| Topic   | Choice                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Layout  | **Desktop (≥768px):** full-width shell with **16px** main padding and a **67px icon sidebar** (Games, game sections, Log out). Sidebar labels use hover/focus tooltips. **Mobile (&lt;768px):** top **header** with hamburger menu, **breadcrumbs**, and a **nav drawer** (labeled links); sidebar hidden; main content full width. Breakpoint tokens in [`src/styles/breakpoints.css`](../src/styles/breakpoints.css). `.appShell` uses a flat page background. Game sections use `.gamePageSection` / `.gamePageStack`. Tasks use **card rows** on small screens; filters collapse behind a compact **Show/Hide filters** control (stacked label → add select → pills when open; task list stays visible below). Planning lets users **show/hide the document tree** (preference stored in `localStorage`) so the editor uses full width. |
...
| PWA     | [`vite-plugin-pwa`](../vite.config.ts) — `skipWaiting` + `clientsClaim`; reload on update via [`src/main.tsx`](../src/main.tsx). After production deploy, users may need to unregister the service worker once (see [`docs/DEPLOY_AUTH0.md`](../../docs/DEPLOY_AUTH0.md)).                                                                                                                                                                                                                                                                                                                                                                                                |
