...
- **Git is advice-only**: never create/switch branches, commit, tag, or push. Propose exact commands and **ask the user** to run them. Delegate git **guidance** to **`release-manager`**; release-manager runs git **only** when the user explicitly approves or mentions the action in chat.
...
- **release-manager**: [`release-manager`](../release-manager/SKILL.md) persona—per-app SemVer, branch/commit/tag **guidance**, trigger-file bumps, CI release verification, commit-message suggestions. Does not implement product code. **Does not** run git unless the user explicitly approves or mentions branch/commit/tag in chat. Checks for an existing branch for the same feature/purpose before proposing a new one.
...
- **Phase 0 — Branch setup** (always first TODO when the plan leads to implementation; **proposed git steps**, not auto-executed):
  - **Owner**: release-manager
  - **Deliverable**: proposed branch name (`feature/<slug>` or reuse of existing branch) + exact git commands; reuse existing branch if one matches the same feature/purpose
  - **Acceptance**: user created/switched branch **or** confirmed reuse of existing branch; no duplicate branch for same purpose; slug documented in the plan; unrelated dirty state stashed or explicitly carried
  - **Slug**: kebab-case from plan title (e.g. `feature/auth0-extraction`)
  - **Note**: include Phase 0 in the plan; **do not assume** the agent will create the branch without user approval
...
- **TODOs**: Phase 0 (release-manager — proposed git steps, user-gated) + 3–8 implementation items; each with Owner/Deliverable/Acceptance criteria/Touched paths.
