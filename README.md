...
- **Auth0: `bad-precaching-response` (404 on `assets/*.js`)** — incomplete deploy or stale service worker. Rebuild frontend, verify asset exists in container, then unregister SW and clear site data for your production app host (see `.env.deploy`). See [docs/DEPLOY_AUTH0.md](./docs/DEPLOY_AUTH0.md).
