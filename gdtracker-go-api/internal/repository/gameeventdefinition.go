...
func (r *GameEventDefinitionRepository) ListByGameOrderByCodeAsc(ctx context.Context, gameID string) ([]GameEventDefinitionRow, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, game_id, code, display_name, message_template, image_data, color
...
func scanEventDefinitionRows(rows *sql.Rows) ([]GameEventDefinitionRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&d.ID, &d.GameID, &d.Code, &d.DisplayName, &d.MessageTemplate, &d.ImageData, &d.Color); err != nil {
			return nil, err
		}
		out = append(out, d)
	}
...
func (r *GameEventDefinitionRepository) FindByIDAndGame(ctx context.Context, id, gameID string) (*GameEventDefinitionRow, error) {
	...
	err := r.db.QueryRowContext(ctx,
		...
		id, gameID,
	).Scan(&d.ID, &d.GameID, &d.Code, &d.DisplayName, &d.MessageTemplate, &d.ImageData, &d.Color)
...
func (r *GameEventDefinitionRepository) Insert(ctx context.Context, id, gameID, code string, displayName sql.NullString, messageTemplate string, imageData sql.NullString, color string) error {
	var disp any
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO game_event_definitions (id, game_id, code, display_name, message_template, image_data, color)
		 VALUES ($1, $2, $3, $4, $5, $6, $7)`,
		id, gameID, code, disp, messageTemplate, imageData, color,
	)
	if err != nil {
...
func (r *GameEventDefinitionRepository) Update(ctx context.Context, id, gameID, code string, displayName sql.NullString, messageTemplate string, imageData sql.NullString, color string) error {
	var disp any
	if displayName.Valid {
		disp = displayName.String
	} else {
	...
	res, err := r.db.ExecContext(ctx,
		`UPDATE game_event_definitions SET code = $3, display_name = $4, message_template = $5, image_data = $6, color = $7
